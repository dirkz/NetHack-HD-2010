...
- (void)awakeFromNib {
	...
	currentVisiblePopovers = [[NSMutableArray alloc] init];
	instance = self;
}
...
- (void)inventoryMenuAction:(id)sender {
	[self presentPopover:self.inventoryPopoverController
	   fromBarButtonItem:sender permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
}
...
- (void)infoMenuAction:(id)sender {
	...
	[self presentPopover:self.actionPopoverController
	   fromBarButtonItem:sender permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
}
...
- (void)tilesetMenuAction:(id)sender {
	...
	[self presentPopover:tilesetPopoverController
	   fromBarButtonItem:sender permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
}
...
- (void)toolsMenuAction:(id)sender {
	ToolsViewController *toolsViewController = [[ToolsViewController alloc]
												initWithStyle:UITableViewStylePlain];
	if (toolsViewController.hasItems) {
		...
		[self presentPopover:toolsPopoverController
		   fromBarButtonItem:sender permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
	}
}
...
- (void)wizardMenuAction:(id)sender {
	NSArray *commands = [NSArray arrayWithObjects:
						 ...
						 [PopoverNhCommand commandWithTitle:"Identify" key:C('i') popover:self.actionPopoverController],
...
- (void)optionsViewAction:(id)sender {
	[self presentPopover:self.optionsPopoverController
	   fromBarButtonItem:sender permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
}
...
- (void)buildToolBar {
	static NSUInteger toolsButtonIndex = -1; // -1 for not there
	if (bottomToolbar.items.count == 1) { // take the one spacer into account that will be there
		// get spacer
		...
		[toolbarItems addObject:[self buttonWithTitle:@"Tiles" target:self action:@selector(tilesetMenuAction:)]];
		
		if (wizard) { // wizard mode
			[toolbarItems addObject:[self buttonWithTitle:@"Wizard" target:self action:@selector(wizardMenuAction:)]];
		}
	...
	// check for tools
	UIBarButtonItem *toolsButton = [self buttonWithTitle:@"Tools" target:self action:@selector(toolsMenuAction:)];
	NSMutableArray *toolbarItems = [NSMutableArray arrayWithArray:bottomToolbar.items];

	NhInventory *inventory = [NhInventory inventory];
	[inventory update];
	if ([inventory containsObjectClass:TOOL_CLASS]) {
		// tools in inv
		if (toolsButtonIndex == -1) {
			[toolbarItems addObject:toolsButton];
			[bottomToolbar setItems:toolbarItems animated:YES];
			toolsButtonIndex = toolbarItems.count - 1;
		}
	} else {
		// no tools in inv
		if (toolsButtonIndex != -1) {
			[toolbarItems removeObjectAtIndex:toolsButtonIndex];
			[bottomToolbar setItems:toolbarItems animated:YES];
			toolsButtonIndex = -1;
		}
	}
...
- (void)displayWindow:(NhWindow *)w {
	if (![NSThread isMainThread]) {
		if (w.type == NHW_TEXT && !w.blocking) {
			// there are text windows (e.g. help > options) that are not blocking but really should
			w.blocking = YES;
		}
...
- (void)handleMapTapTileX:(int)x y:(int)y forLocation:(CGPoint)p inView:(UIView *)view {
	...
	if (directionQuestion) {
		if (u.ux == x && u.uy == y) {
			...
			[self presentPopover:self.actionPopoverController fromRect:tapRect
...
- (void)dismissAllVisiblePopovers {
	for (UIPopoverController *popover in currentVisiblePopovers) {
		[self dismissIfVisiblePopover:popover];
	}
	[currentVisiblePopovers removeAllObjects];
}
...
- (void)presentPopover:(UIPopoverController *)popover
	...
	[self dismissAllVisiblePopovers];
	...
	[currentVisiblePopovers addObject:popover];
}

- (void)presentPopover:(UIPopoverController *)popover fromRect:(CGRect)rect
				inView:(UIView *)view permittedArrowDirections:(UIPopoverArrowDirection)arrowDirections animated:(BOOL)animated {
	[self dismissAllVisiblePopovers];
	[self resizePopover:popover];
...
- (void)dealloc {
	[currentVisiblePopovers release];
    [super dealloc];
}
